...
class TargetGroup(BaseModel):
    target_group: str = Field(..., description="Description of the target group of the video in polish language.")
...
def extract_target_group(transcription_words: List[TranscriptionWord]) -> str:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "Your task is to describe the target group of the video in polish language. Focus on education level and age. Be concise (max 2 sentences).\n{output_format}"),
