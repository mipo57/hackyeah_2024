...
class Keywords(BaseModel):
    keywords: List[str] = Field(..., description="List of keywords best describing the content of the transcript")
...
def extract_keywords(transcription_words: List[TranscriptionWord]) -> List[str]:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "Your task is to generate keywords describing the content of the transcript (from 4 to 6). Keywords should be short, simple, unique and not related to each other.\n{output_format}"),
