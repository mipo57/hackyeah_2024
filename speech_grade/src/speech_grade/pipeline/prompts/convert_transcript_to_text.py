...
class Transcription(BaseModel):
    readable_transcription: str = Field(
        ..., description="Transcription with ASR errors fixed and proper punctuation"
    )
...
def convert_transcript_to_text(transcription_words: List[TranscriptionWord]) -> str:
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "Your task is to convert output of ASR to text with ASR errors fixed and proper punctuation.\n{output_format}",
            ),
