...
from speech_grade.pipeline.prompts.convert_transcript_to_text import convert_transcript_to_text
...
def step_transcribe_audio(state: State) -> State:
    return {
        "transcription_words": transcribe_audio(state["audio_path"])
    }

def step_convert_transcript_to_text(state: State) -> State:
    return {
        "readable_transcription": convert_transcript_to_text(state["transcription_words"])
    }
...
def route_classify_image(state: State) -> State:
    files = [f for f in os.listdir(state["frames_dir_path"]) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp'))]

    print(files)

    return [Send("step_classify_image", {"image_path": os.path.join(state["frames_dir_path"], image_file)}) for image_file in files]
...
def step_extract_keywords(state: State) -> State:
    return {
        "keywords": extract_keywords(state["transcription_words"])
    }

def step_generate_questions(state: State) -> State:
    return {
        "questions": generate_questions(state["transcription_words"])
    }

def step_analyze_speech_volume(state: State) -> State:
    high_volume_words, low_volume_words, volumes, volumes_timestamps = analyze_speech_volume(state["audio_path"], state["transcription_words"])
...
def step_generate_suggestions(state: State) -> State:
    return {
        "suggestions": generate_suggestions(state["transcription_words"], state["events"])
    }

def step_extract_named_entities(state: State) -> State:
    return {
        "named_entities": extract_named_entities(state["transcription_words"])
    }
...
def step_extract_target_group(state: State) -> State:
    return {
        "target_group": extract_target_group(state["transcription_words"])
    }

def step_classify_sentiment(state: State) -> State:
    return {
        "sentiment": classify_sentiment(state["transcription_words"])
    }
...
def step_calculate_speech_speed(state: State) -> State:
    ...
    avg_words_per_minute, words_per_minute, wpm_timestamps, pauses, pauses_timestamps = speech_speed(state["transcription_words"])
        ...
        for wpm, timestamp in zip(words_per_minute[1:], wpm_timestamps[1:]):
            if wpm < MIN_WPM:
                events.append(Event(
                    start_s=timestamp[0],
                    end_s=timestamp[1],
                    event=f"Niska szybkość mówienia",
                ...
                events.append(Event(
                    start_s=timestamp[0],
                    end_s=timestamp[1],
                    event=f"Wysoka szybkość mówienia",
                    description=f"(WPM: {int(wpm)})\n" + description,
                    color="#F44336"
                ))
...
def step_add_clarity_score(state: State) -> State:
    ...
    if fog_index > HARD_FOG_THRESHOLD:
        ...
        events = [Event(
            ...
            event=f"Mglista wypowiedź (Wysoki Fog Index)",
...
def build_graph():
    ...
    graph_builder.add_node("step_extract_audio", step_extract_audio, retry=DEFAULT_RETRY_POLICY)
    graph_builder.add_node("step_transcribe_audio", step_transcribe_audio, retry=DEFAULT_RETRY_POLICY)
    graph_builder.add_node("step_extract_frames", step_extract_frames, retry=DEFAULT_RETRY_POLICY)
    graph_builder.add_node("step_classify_image", step_classify_image, retry=DEFAULT_RETRY_POLICY)
    graph_builder.add_node("step_gather_images", step_gather_images, retry=DEFAULT_RETRY_POLICY)
    graph_builder.add_node("step_detect_audio_problems", step_detect_audio_problems, retry=DEFAULT_RETRY_POLICY)
    graph_builder.add_node("step_add_formatted_transcription", step_add_formatted_transcription, retry=DEFAULT_RETRY_POLICY)
    graph_builder.add_node("step_convert_transcript_to_text", step_convert_transcript_to_text, retry=DEFAULT_RETRY_POLICY)
    ...
    graph_builder.add_node("step_extract_target_group", step_extract_target_group, retry=DEFAULT_RETRY_POLICY)
    graph_builder.add_node("step_translate_to_english", step_translate_to_english, retry=DEFAULT_RETRY_POLICY)  
    graph_builder.add_node("step_extract_named_entities", step_extract_named_entities, retry=DEFAULT_RETRY_POLICY)
    graph_builder.add_node("step_generate_suggestions", step_generate_suggestions, retry=DEFAULT_RETRY_POLICY)
    graph_builder.add_node("step_classify_sentiment", step_classify_sentiment, retry=DEFAULT_RETRY_POLICY)
    graph_builder.add_node("step_analyze_speech_volume", step_analyze_speech_volume, retry=DEFAULT_RETRY_POLICY)
    graph_builder.add_node("step_add_clarity_score", step_add_clarity_score, retry=DEFAULT_RETRY_POLICY)
    graph_builder.add_node("step_generate_questions", step_generate_questions, retry=DEFAULT_RETRY_POLICY)
    ...
    graph_builder.add_conditional_edges("step_extract_frames", route_classify_image, ["step_classify_image"])
    ...
    graph_builder.add_edge("step_convert_transcript_to_text", "step_translate_to_english")
